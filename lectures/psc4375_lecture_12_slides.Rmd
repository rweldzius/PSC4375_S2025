...
\pause
  ...
  - Prediction error compared to what?\pause
    - Baseline prediction error: **Total sum of squares** TSS $= \sum_{-=1}^{n}(Y-\bar{Y})^2$\pause
    - Model prediction error: **Sum of squared residuals** SSR $= \sum_{-=1}^{n} \epsilon_i^2$\pause
    - TSS - SSR: reduction in prediction error by the model.\pause
  - $R^2$ is this reduction in error divided by the baseline error: \pause
